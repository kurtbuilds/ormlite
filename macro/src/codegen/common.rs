...
fn recursive_primitive_types_ty<'a>(ty: &'a attr::TType, cache: &'a MetadataCache) -> Vec<Cow<'a, attr::InnerType>> {
    match ty {
        attr::TType::Option(ty) => {
        ...
        attr::TType::Inner(p) => vec![Cow::Borrowed(p)],
        attr::TType::Join(j) => {
